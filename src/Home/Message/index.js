    ...
    render() {
        ...
        return <div className={"p-mt-5"}>
            ...
            <div className="p-d-flex p-jc-between p-ai-center">
                ...
                <div>
                    <Button onClick={() => { this.setPostWindow(true) }} className={"p-button-primary"} style={{float: "right"}} icon="pi pi-plus" iconPos="right" />
                </div>
            </div>

            {this.state.replies.map(reply => {
                return <Card className={"p-mb-2"} subTitle={<span><span style={{color: "blue"}}>@{reply.author}</span> op {new Date(reply.created).toLocaleString()}</span>}>
